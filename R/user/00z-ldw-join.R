
## Issue here is that land use class is needed to assign wood density 
## but land use class is recorded for each land cover section
## Solution: assign center LCS to subplot.

## Get center LCS to proxy subplot land use class

tmp <- list()

tmp$subplot_lu <- lcs |>
  filter(lcs_no == 1) |> 
  select(lcs_plot_no, lcs_subplot_no, lcs_lu_class_txt) |>
  arrange(lcs_plot_no, lcs_subplot_no)


## JOINS ####

## Extract codes from subplot table
tmp$subplot_codes <- subplot |> select(ONA_index, subplot_plot_no, subplot_no, subplot_lc_class_center)

## plot, subplot and LCS codes ####
ldw <- ldw |>
  mutate(
    subplot_plot_no = NA, 
    subplot_no = NA, 
    subplot_lc_class_center = NA 
  ) |>
  left_join(tmp$subplot_codes, by = join_by(ONA_parent_index == ONA_index), suffix = c("_rm", "")) |>
  select(-ends_with("_rm"))


## LU class ####
ldw <- ldw |>
  mutate(lcs_lu_class_txt = NA) |>
  left_join(
    y = tmp$subplot_lu, 
    by = join_by(subplot_plot_no == lcs_plot_no, subplot_no == lcs_subplot_no),
    suffix = c("_rm", "")
    ) |>
  select(-ends_with("_rm")) |>
  mutate(
    lcs_lu_class_no = as.numeric(case_when(
      lcs_lu_class_txt == "16AC" ~ "161",
      lcs_lu_class_txt == "16EC" ~ "162",
      lcs_lu_class_txt == "16PN" ~ "163",
      lcs_lu_class_txt == "16RB" ~ "164",
      lcs_lu_class_txt == "16TK" ~ "165",
      lcs_lu_class_txt == "16OTH" ~ "169",
      TRUE ~ lcs_lu_class_txt
    ))
  )

## LC names ####
ldw <- ldw |>
  mutate(
    lu_code_new = NA,
    lu_name = NA,
    lu_name_lao = NA,
    lu_type = NA,
    lu_strata_no = NA,
    lu_strata_name = NA
  ) |>
  left_join(anci$lc, by = join_by(lcs_lu_class_no == lu_no), suffix = c("_rm", "")) |>
  select(-ends_with("_rm"))



## save table in 'results'
write_csv(ldw, file.path(path$res$data, paste0("ldw_with_joins-", Sys.Date(), ".csv")))  

## Remove tmp object
rm(tmp)
