...
This section is illustrated with an example from a subset of 5 plots from the data (see @exm-pres).

::: {.callout-tip appearance="simple"}
## Set example {#exm-pres}

Let's take example of plot no. 1 and the calculation of AGB.

XX INSERT TABLE

plot no.1 is composed of 20 subplots, 18 in DD, and 2 in RV. When aggregation subplots to plot level, there will be 2 partial plots $1_{DD}$ and $1_{RV}$ . In this example:

$$
\text{AGB}_{1,DD} = \sum_{j=A1}^{D5} AGB_{1,j,DD} 
$$

and

$$
\text{AGB}_{1,RV} = \sum_{j=A1}^{D5} AGB_{1,j,RV} 
$$
:::
