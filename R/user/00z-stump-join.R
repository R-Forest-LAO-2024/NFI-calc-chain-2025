...
## NEW METHOD: INITIATE JOINED COL AND CONTROL SUFFIX TO AVOID ERROR WHEN JOINING TWICE THE SAME COLS
...
stump <- stump |>
  ...
  left_join(
    y = lcs, 
    by = join_by(subplot_plot_no == lcs_plot_no, subplot_no == lcs_subplot_no, stump_lcs_no == lcs_no),
    suffix = c("_rm", "")
  ) |>
  select(-ends_with("_rm")) |>
  mutate(
