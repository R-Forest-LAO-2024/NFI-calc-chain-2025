...
## + All LCS theoretically measured ####
lcs_all <- expand_grid(
...
## Updating LCS to match the Est. tool
## Create a duplicate of 553 to match Est. tool stats
tmp$dup_553 <- lcs_all |> filter(plot_no == 553) #|> mutate(plot_no = 10553)

## If duplicate not yet added, add it
if (nrow(filter(lcs_all, plot_no == 553)) == 20) {
  lcs_all <- lcs_all |> bind_rows(tmp$dup_553)
}

## Filter out omitted plots
lcs_all <- lcs_all |> filter(!plot_no %in% c(106, 139, 154, 439, 534, 578, 1163))

## Check matching Est. tool without QAQC: 21220 
message("LCS samples adjusted for Est. tool: ", nrow(lcs_all) == 21220)
## !!!
...
## Check
table(lcs_all$subpopulation_no, useNA = "ifany")
...
lcs_all <- lcs_all |>
  mutate(
    ...
    subplot_access2 = case_when(
      plot_no %in% tmp$subplot_notvisited & ceo_plot_stratum_no == 1 ~ FALSE,
      plot_no %in% tmp$subplot_notvisited & ceo_plot_stratum_no == 2 ~ FALSE,
      plot_no %in% tmp$subplot_notvisited & ceo_plot_stratum_no == 3 ~ TRUE,
...
table(lcs_all$ceo_plot_stratum_no, lcs_all$subplot_access1, useNA = "ifany")
## >> Big difference with est. tool :'(

## Adding not visited plots, see subplot_access2
table(lcs_all$subplot_access2, useNA = "ifany")
table(lcs_all$ceo_plot_stratum_no, useNA = "ifany")
table(lcs_all$ceo_plot_stratum_no, lcs_all$subplot_access2, useNA = "ifany")
...
## Testing adding shifted plot CEO to align. 
...
## Check
table(lcs_all$ceo_plot_stratum_no_corr, lcs_all$subplot_access_final, useNA = "ifany")
## >> Yes! full match
