...
-   Aggregation to domain estimates: aggregation of entity varaibles to domqi estimates with uncertainty of sampling error. Aggregation levels:
...
knitr::kable(tbl_entity, format.args = list(big.mark = ','))
...
knitr::kable(tbl_carbon, format.args = list(big.mark = ","))
...
The initial scripts were developed to retrieve the data directly from the ONA server into R as one master CSV (see ONA API instructions: <https://api.ona.ilri.org/static/docs/data.html>), or from manually downloaded tables as a ZIP file or as a CSV with long names (see ONA interface download options).
...
Nevertheless, it was decided that the data would be downloaded manually by FIPD and shared with experts and analysts. In this case, the data was stored in the following directory inside the R project: `'data/data-source'`, and user inputs were set as follows in `'R/user/00-user-input.R'` (Code @lst-usrinput).
...
At subplot level the main type of issue found was entry error of subplot code (duplicates in A, B, C or D) and plot code. They were solved manually by looking at time stamps. The full list of corrections were coded in `'R/user/00a-clean-subplot.R'`, a snippet is presented in Code @lst-spcorr.
