...
## Introducing the guiding example {#exm-intro}

To illustrate the formulas and step-by-step aggregation of the NFI data from entities to domain totals, a subset of 5 phase 2 plots (2 DD, 2 RV, 1 mix) and 20 phase 1 plots (15 RV, 5 DD) was prepared (see @tbl-guidex-ph1 and @tbl-guidex-ph2init. The guiding example was also limited to live trees and AGB aggregation, but the principles are the same for other entities and attributes. All the initial tree data is presented in @sec-guidex.

```{r}
#| echo: false
#| label: tbl-guidex-ph1
#| tbl-cap: "Guiding example Phase 1 subset."
...
#| tbl-cap: "Guiding example Phase 2 initial subset."

knitr::kable(guidex$ph2_init, format.args = list(big.mark = ','))
...
In the NFI cycle 4, the minimal unit area was the subplot's land cover section, i.e. the area defined by setting equidistance between the five land cover observation points for each subplot. In the whole aggregation section, subplots, land cover or subplot x land cover were used interchangeably to refer to this minimal area unit that constitute the initial aggregation of entities to a measurement area.

The initial `ph2_subplot` table was generated from all possible combinations of phase 2 plot numbers, subplot numbers (A, B, C or D) and land cover section (1 to 5).

Each entity table's variables were then sumed to the subplot level.
...
A_{i,j} = 
\begin{cases} 
12^2/10000 = 0.0144 \space ha \text{, for center land cover section} \\
(\pi \times 16^2 - 12^2) / 40000 = 0.0165 \space ha \text{, for other land cover sections}
\end{cases}
$$ {#eq-lcs-area}
...
Here the tree AGB was summed to the subplot level either in R with `group_by()` and `summarise()` or in MS Excel with SUMIFS() or pivotTable. The tree aggregated data was then combined with the initial phase 1 subplot data. In R use `left_join()` from the subplot initial table to make sure all subplots are there even if there are no trees (see @tbl-guidex-ph2).
...
#| tbl-cap: "Phase 2 subplot with tree AGB and area."
...
$$
y_{i,d} = \sum_j \delta_j \times y_{i,j,d}
$$
...
# Annex: Data for the guiding example {#sec-guiding-example}

To help understand the aggregation process, a subset of 20 phase 1 plots and 5 phase 2 plots were used. The phase 2 plots include 1 mixed land use plot: 18 subplots x land cover sections in Dry Dipterocarp forests (DD) and 2 subplots x land cover sections in Regenerative Vegetation (RV). The 4 other plots are pure, 3 in Dry Dipterocarp and 1 in Regenerative Vegetation. The 20 Phase 1 plots we selected to include the Phase 2 plots are respect the ratio of RV to DD, which is around 3/4 RV, 1/4 DD in the full dataset (see @tbl-ex-ph1 and @tbl-ex-tree).

```{r}
#| echo: false
#| label: tbl-ex-ph1
#| tbl-cap: "Phase 1 information for the guiding example"
...
#| echo: false
#| label: tbl-ex-tree
#| tbl-cap: "Phase 2 tree measurements for the guiding example"
