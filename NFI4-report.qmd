...
# Data analysis: Preparation of the data

## Data archiving and structure for the R project
...
# Data analysis: entity level calculations
...
```{r}
#| echo: false
#| label: tbl-guidex-ph2init-10rows
...
The initial `ph2_subplot` table was generated from all possible combinations of phase II plot numbers, subplot numbers (A, B, C or D) and land cover section (1 to 5). The area of each subplot was calculated and each entity variables were then summed to the subplot level.

### Area of each subplot $A_{i,j}$
...
A_{i,j} = 
\begin{cases} 
12^2/10000 = 0.0144 \space \text{ ha, for the center land cover section} \\
(\pi \times 16^2 - 12^2) / 40000 = 0.0165 \space \text{ ha, for outer land cover sections}
\end{cases}
$$ {#eq-lcs-area}

### Tree, deadwood and stump measurements $(X_y)_{i,j}$
...
$$
(X_y)_{i,j} = \sum_k{(X_y)_{i,j,k} \times w_y}
$$

with $X_y$ the calculated variable $X$ (ex. biomass or basal area) of the entity $y$ (tree, stump or deadwood), $i,j,k$ the plot and subplot and entity number respectively (ex. plot 1, subplot 1A, tree 1) and $w_y$ the weights (See @eq-treeweight for trees, standing deadwood and stumps).

```{=html}
<!-- 
::: {.callout-tip appearance="simple"}
<!-- #02 guidex aggregation to subplot -->
```
...
### Sapling measurements $(X_\text{sap})_{i,j}$

Sapling were measured in a 2m radius circle located in the center land cover section. To account for the fact that the sapling measurement only reflect the land cover of the center land cover section, their measurement were applied only to the partial plots of the center land cover section. For other land cover sections, the amount of sapling was considered 0.

To extrapolate the number of sapling from the 2m radius measurement subplot to the land cover sections the calculation sequence was as follows:

Filter the

### Lying Deadwood $(X_\text{ldw})_{i,j}$
